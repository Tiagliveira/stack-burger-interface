  ...
  return (
    <>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell >
          <SelectStatus
            options={orderStausOptions.filter((status) => status.id !== 0)}
            placeholder='Status'
            defaultValue={orderStausOptions.find(status => status.value === row.status || null)}
                ...
                <TableBody>
                  {row.products.map((product) => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell > <ProductImage src={product.url} alt={product.name} /></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
